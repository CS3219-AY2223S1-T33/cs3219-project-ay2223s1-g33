...
class CreateQuestionHandler
	implements IApiHandler<CreateQuestionRequest, CreateQuestionResponse>
{
	...
	async handle(
		...
		if (!request.question) {
			return CreateQuestionHandler.buildErrorResponse(
    ...
    if (request.question.difficulty === QuestionDifficulty.UNUSED || 
